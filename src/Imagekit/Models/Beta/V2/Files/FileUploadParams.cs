...
using Imagekit.Models.Beta.V2.Files.FileUploadParamsProperties.SelectedFieldsSchemaProperties;
...
/// uploads, generate a one-time `token` from your secure backend using private API.
...
/// limits increase to 40MB for images, audio, and raw files, and 2GB for videos.
/// These limits can be further increased with higher-tier plans.
///
...
public sealed record class FileUploadParams : ParamsBase
{
    ...
    /// error. Even if your previous request resulted in an error, you should always
    /// send a new token.
    ///
    ...
    {
        return new UriBuilder(client.BaseUrl.ToString().TrimEnd('/') + "/api/v2/files/upload")
        {
