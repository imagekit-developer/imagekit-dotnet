...
public interface IFileService
{
    ...
    /// API is in beta.
    ///
    /// <para>ImageKit.io allows you to upload files directly from both the server
    /// and client sides. For server-side uploads, private API key authentication
    /// is used. For client-side uploads, generate a one-time `token` from your secure
    /// backend using private API. [Learn more](/docs/api-reference/upload-file/upload-file-v2#how-to-implement-secure-client-side-file-upload)
    ...
    /// are 20MB for images, audio, and raw files, and 100MB for videos. On the paid
    /// plan, these limits increase to 40MB for images, audio, and raw files, and
    /// 2GB for videos. These limits can be further increased with higher-tier plans.</para>
    ///
...
public interface IFileServiceWithRawResponse
{
    ...
    /// <summary>
    /// Returns a raw HTTP response for `post /api/v2/files/upload`, but is otherwise the
